...
export default class TrainController implements Controller<Entity<Train>, Entity<Operator> | { admin: true }> {
    public resourceName: string = "trains"

    constructor(protected repo: Repository<Train>){}
    
    public async get(id: string, _params: json, _auth?: Entity<Operator> | { admin: true }): Promise<Entity<Train>> {
        const model = await this.repo.getById(id)
        if(!model) throw new HttpErrors.Client.NotFound()
        return model
    }

    public async getAll(_params: json, _auth?: Entity<Operator> | { admin: true }): Promise<Entity<Train>[]> {
        return await this.repo.getAll()
    }

    public async create(model: json, _params: json, auth?: Entity<Operator> | { admin: true }): Promise<Entity<Train>> {
